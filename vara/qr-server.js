...
function getLanIPv4() {
  const fromEnv = process.env.EXPO_LAN_IP || process.env.LOCAL_IP;
  ...
  for (const name of Object.keys(nets)) {
    for (const net of nets[name] || []) {
      ...
      if (isV4 && !net.internal) {
        if (name === 'en0' || name === 'en1') return net.address;
      }
    }
  }
  for (const name of Object.keys(nets)) {
...
// Expo Metro default is 8081 (see `expo start` output). Override with EXPO_PORT.
const LOCAL_IP = getLanIPv4();
const EXPO_PORT = Number(process.env.EXPO_PORT) || 8081;
const QR_SERVER_PORT = Number(process.env.QR_SERVER_PORT) || 3003;
...
server.listen(QR_SERVER_PORT, '0.0.0.0', () => {
  console.log(`QR page (this Mac): http://localhost:${QR_SERVER_PORT}`);
  console.log(`QR page (same Wi‑Fi): http://${LOCAL_IP}:${QR_SERVER_PORT}`);
  console.log(`Expo URL encoded in QR: ${expoUrl}`);
});
